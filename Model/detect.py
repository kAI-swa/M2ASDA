...
class Detect_SC(Detect):
    ...
    def fit(self, train: Union[ad.AnnData, str]):
        ...
        self.D.train()
        self.G.train()
        loop = tqdm(range(self.n_epochs), total=self.n_epochs, leave=False)
        for epoch in loop:
            loop.set_description("Model Training")
            for i in range(self.n_critic):
                self.Update_D()
            self.Update_G()
            loop.set_postfix(G_loss = self.G_loss.item(), D_loss = self.D_loss.item())
