...
def parse_args():
    ...
    parser.add_argument("--temperature", type=float, default=1, help="Temperature for Dot product attention")
    parser.add_argument("--n_critic", type=int, default=1, help="Numeber of updating Discriminator per epoch")
    parser.add_argument("--pretrain", action="store_false", help="Determine whether pretrain before anomaly detection or not")
    parser.add_argument("--gpu", action="store_false", help="Training on GPU by default")
    parser.add_argument("--verbose", action="store_false", help="log")
...
if __name__ == "__main__":
    ...
    precision_scores = []
    recall_scores = []
    with tqdm(total=len(threshold_list), leave=True) as t:
